...
| 005 | Track Copied Files | completed | [005-track-copies.md](005-track-copies.md) | Add copied files to the database during organize --action copy so the DB tracks all known image locations. |
| 006 | API Layer | deferred | [006-api-layer.md](006-api-layer.md) | HTTP/JSON API exposing scan, duplicates, organize, and query operations. Foundation for TUI/GUI clients. |
...
| 011 | Tags & Galleries | planned | [011-tags-galleries.md](011-tags-galleries.md) | Tag images with user-defined labels (N per image) and assign galleries (1 per image). Searchable and usable as organize format tokens. |
...
| 014 | Thumbnails | completed | [014-thumbs.md](014-thumbs.md) | Generate 512px JPEG thumbnails cached on filesystem by SHA256 hash. `phig thumbs` command + `--thumbs` on scan. Granular `--force` rework (hash/thumbs/faces/all). |
| 015 | Video Support | planned | [015-video-support.md](015-video-support.md) | Extend phig to scan, hash, deduplicate, and organize video files. Add FFmpeg for video metadata extraction and keyframe phash. Schema additions for duration/codec/fps. |
| 016 | Favorites | planned | [016-favorites.md](016-favorites.md) | Mark images as favorites with `phig fav add/remove/list`. Boolean flag per image, searchable via `--fav`, usable as `%fav` organize format token. |
